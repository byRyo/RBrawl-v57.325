...
class ByteStream {
  ...
  /**
   * Writing raw hex string to Bytes
   * @param {String} hex Your hex string (e.g., "A1B2C3", "A1-B2-C3", or "a1 b2 c3")
   */
  writeHex (hex) {
    this.bitOffset = 0
  ...
  /**
   * Writing zlib-compressed data to Bytes (Supercell format):
   * Int (compressed length + 4), UInt32LE (uncompressed length), zlib data
   * @param {Buffer|String} data Your data to compress and write.
   */
  writeCompressedString (data) {
    const zlib = require('zlib')
    const raw = Buffer.isBuffer(data) ? data : Buffer.from(data != null ? data : '', 'utf8')
    ...
    lenLE.writeUInt32LE(raw.length, 0)
    this.buffer = Buffer.concat([this.buffer, lenLE, compressed])
    this.offset += 4 + compressed.length
  }

  writeIntList(intList) {
    this.writeVInt(intList.length);
    for (let i = 0; i < intList.length; i++) this.writeVInt(intList[i]);
  }
  ...
  send () {
    ...
    this.encode()
    const payload = this.client.crypto
      ? this.client.crypto.encrypt(this.id, this.buffer)
      : this.buffer
